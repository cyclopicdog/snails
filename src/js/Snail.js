export default class Snail {
    constructor(name, color, speed, distance = 0) {
    ...
    render() {
        this.thing = this.createHTMLelement(
            `<div class="race-track__lane">
        ...
        document.addEventListener('click', (event) => {
            console.log('pressed');
            return this.move_snail(this.speed);
        });
    }
    ...
    update() {
        this.thing.querySelector('.snail__shell').style.fill = this.color;
        this.thing.querySelector('.snail-racer__name').textContent = this.name;
        this.thing.querySelector(
            '.snail__body'
        ).style.transform = `scaleX(${this.scale})`;

        this.thing.querySelector(
            '.snail'
        ).style.transform = `translateX(${this.distance}px)`;
        this.thing.querySelector(
            '.snail__shell'
        ).style.transform = `rotatez(${this.rotation}deg)`;
        console.log(`${this.name} has gone ${this.distance}`);
        console.log(this.scale);
    }

    move_snail(speed) {
        if (this.distance < 520) {
            setTimeout(() => {
                this.distance += speed;
                this.grow_width();
                this.shell_tilt_push();
                this.update();
                setTimeout(() => {
                    this.shrink_width();
                    this.shell_tilt_pull();
                    this.update();
                    this.move_snail(speed);
                }, 500);
            }, 500);
        }
    }
